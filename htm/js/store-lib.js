...
var sF$=(function(){
      ...
      for(var i=0;i<aImgsShare.length;i++){
        ...
        if(aImgsShare[i].className=='EstImgShareFacebook'){
          ...
          if(fnGetConfig("Instagram_Page_Link")){
            aImgsShare[i].setAttribute(sSrc,FC$.PathImg +'iconprodinstagram.svg?cccfc=1');
          ...
          if(fnGetConfig("Youtube_Page_Link")){
            aImgsShare[i].setAttribute(sSrc,FC$.PathImg +'iconprodyoutube.svg?cccfc=1');
  ...
  function fnSlideshowStore(aBanners){
    var numBannerHomeFC=fnGetConfig("Qty_Banner_Slideshow"),oSwiper=document.getElementById("fc-dynamic-swiper");
    for(i=0;i<numBannerHomeFC && i<aBanners.length;i++){
      oBanner=aBanners[i];
      oSwiper.innerHTML+='<div class=swiper-slide><a href="/banner-redir.ehc?idbanner='+ oBanner.bannerID +'&url='+ encodeURIComponent(oBanner.url) +'"><picture><source media="(max-width:1024px)" srcset="'+ oBanner.imageMob +'"><img alt="'+ oBanner.alt.replace(/"/g,'&quot;') +'" src="'+ oBanner.image +'"></picture></a></div>';
    }
    FCLib$.execLazyLoad();
  }
  function fnSlideshowSwiper(aBanners){
    ...
    var swiper=new Swiper('.swiper-container',{
      ...
      on:{
        slideChangeTransitionEnd:function(swiper){
          var oCurrentSlide=swiper.slides[swiper.realIndex];
          if(oCurrentSlide)incrementBannerPrints(aBanners[swiper.realIndex].bannerID);
        }
      }
    });
  }
...
function fnFooter(){
  ...
  FCLib$.onReady(function(){
    ...
    if(FC$.Page=="Products"){
      ...
      sF$.fnSlideshowSwiper(aBannersHome);
